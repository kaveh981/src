...
/**
 * @case    - A buyer views proposals they own, an open one and an expired one.
 * @expect  - The buyer should receive both proposals.
 * @route   - GET deals/proposals/owned
 * @status  - working
 * @tags    - get, deals
 */
export async function ATW_API_GET_DEAPROOUT_FUNC_08 (assert: test.Test) {
    ...
    let openProposal = await databasePopulator.createProposal(buyerCompany.user.userID, [ section.section.sectionID ]);
    let expiredProposal = await databasePopulator.createProposal(buyerCompany.user.userID, [ section.section.sectionID ],
                                                                 { startDate: '1992-07-29', endDate: '2000-12-01' });
    let strayProposal = await databasePopulator.createProposal(pubCompany.user.userID, [ section.section.sectionID ], {}, [ buyerCompany.user.userID ]);
    ...
    assert.equals(response.status, 200);
    assert.deepEquals(response.body['data'], [ Helper.proposalToPayload(openProposal, buyerCompany.user),
                                               Helper.proposalToPayload(expiredProposal, buyerCompany.user) ]);
